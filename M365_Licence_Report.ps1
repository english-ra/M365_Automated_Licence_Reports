...
foreach ($user in $users) {
    ...
    foreach ($licence in $user.AssignedLicenses) {
        ...
        #Display error if licence name can't be found
        if ( $licenceDetails.Product_Display_Name -eq $null ) { $licenceNames += "[UNABLE TO LOCATE LICENCE NAME]" }
        else { $licenceNames += $licenceDetails.Product_Display_Name }
        $licenceNames += ", "
        $licenceCount += 1
 
        Write-Output($licence.skuId)
        Write-Output($licenceNames)
    }
...
$params = @{
    Message = @{
        Subject = "M365 Licence Report - " + $clientName + " - " + $date
        Body = @{
            ContentType = "Text"
